    ...
    <div class="blog-post">
        <div <?php post_class();?>>
            ...
            if(isset($response[0]) && $response[0] != "") {
                $no_img = '';
                ?><div class="post-img"><a href="<?php the_permalink();?>"><img class="img-responsive" src="<?php echo esc_url($response[0]);?>" alt="<?php the_title();?>"></a></div>
                <?php }  ?>
            ...
            <?php  if (get_the_excerpt() != "") { ?><p class="post-excerpt"><?php echo get_the_excerpt();?><a href="<?php the_permalink();?>"><strong><?php echo __('Read More', 'adforest');?></strong></a></p><?php }  ?>
        </div>
    </div>
</div>
        <?php
