    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-3  col-md-4 col-lg-4 ">
                <div class="sb-mob-top-icons-area"><ul class="list-inline sb-top-main-contents"><?php foreach ($adforest_theme['social_media'] as $index => $val) { ?> <?php if ($val != "") { ?> <li><a <?php do_action('adforest_relation_follow_links');?>href="<?php echo esc_url($val);?>"><i class="<?php echo adforest_social_icons($index);?>"></i></a></li><?php }  } ?></ul></div>
            </div>
        </div>
    </div>
</section>
                                ...
                                ?><li><div class="sb-adf-header">
                                        ...
                                        <form class="form-inline" method="get" action="<?php echo urldecode(get_the_permalink($sb_search_page));?>">
                                            <div class="form-group">
                                                <label class="sr-only" for="lokking-for"><?php echo __('Email address', 'adforest');?></label><input id ="lokking-for" placeholder="<?php echo __('What Are You Looking For ?', 'adforest');?>" type="text" name="ad_title" class="form-control" value="<?php echo esc_attr($search_title);?>" autocomplete="off">
                        ...
                        if (isset($adforest_theme['search_in_header']) && $adforest_theme['search_in_header'] && wp_is_mobile()) {
                            ...
                            <div class="sb-adf-header">
                                ...
                                <form class="form-inline" method="get" action="<?php echo urldecode(get_the_permalink($sb_search_page));?>">
                                    <div class="form-group">
                                        <label class="sr-only" for="lokking-for"><?php echo __('Email address', 'adforest');?></label><input id ="lokking-for" placeholder="<?php echo __('What Are You Looking For ?', 'adforest');?>" type="text" name="ad_title" class="form-control" value="<?php echo esc_attr($search_title);?>" autocomplete="off">
                                        <span class="adforest-spinner" style="display:none"><i class="fa fa-spinner spin"></i></span>
                                    </div>
                                    <?php apply_filters('adforest_form_lang_field', true);?>
                                    <button class="btn btn-theme" type="submit"><i class="fa fa-search"></i></button>
                                </form>
                                ...
                                <li class="sb-new-sea-green sb-nav-table dropdown"> 
                                    ...
                                    <ul class="dropdown-menu sb-user-pro">
                                        ...
                                        <?php if (isset($adforest_theme['sb_cart_in_menu']) && $adforest_theme['sb_cart_in_menu'] && in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                                            global $woocommerce; ?><li><a href="<?php echo wc_get_cart_url();?>"><?php echo __('Cart', 'adforest');?><span class="badge"><?php echo adforest_returnEcho($woocommerce->cart->cart_contents_count);?></span></a></li><?php } ?>
                                        <li role="separator" class="divider"></li>
                                        <li><a href="<?php echo wp_logout_url(get_the_permalink($sb_sign_in_page));?>">
                                                <?php echo __("Logout", "adforest");?></a></li></ul></li>
                                <?php }  } else {
                            if (isset($adforest_theme['sb_sign_up_page']) && $adforest_theme['sb_sign_up_page'] != "") { ?><li class="sb-bg-blue p-relative"><a href="<?php echo get_the_permalink($sb_sign_up_page);?>"><i class="fa fa-unlock"></i></a></li><?php }
