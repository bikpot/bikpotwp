                ...
                if ($adforest_theme['communication_mode'] == 'both' || $adforest_theme['communication_mode'] == 'message') {
                    ...
                    <div class="<?php echo esc_attr($message_col); ?>">
                        <?php if (get_current_user_id() == "") { ?>
                            <a href="<?php echo get_the_permalink($sb_sign_in_page); ?>" class="btn btn-block <?php echo esc_attr($msg_cls); ?> btn-message"><i class="icon-envelope"></i> <?php echo __('Message Seller', 'adforest'); ?></a>
                            <?php } else { ?>
                            <a data-toggle="modal" data-target=".price-quote"  href="javascript:void(0)" class="btn btn-block <?php echo esc_attr($msg_cls); ?> btn-message"><i class="icon-envelope"></i> <?php echo __('Send Message', 'adforest'); ?></a>
                            <?php } ?>
                    </div>
                    <?php } ?>
            </div>
