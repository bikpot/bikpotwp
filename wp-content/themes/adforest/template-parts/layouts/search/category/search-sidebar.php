...
<div class="main-content-area clearfix">
    <section class="section-padding <?php echo esc_attr($section_class); ?>">
        <div class="container">
            <div class="row">
                <?php
                $display_cat_desc = isset($adforest_theme['display_cat_desc']) && $adforest_theme['display_cat_desc'] ? TRUE : FALSE;
                $cat_term_desc = term_description($term_id);

                if ($display_cat_desc && isset($term_id) && $term_id != '' && $cat_term_desc != '') {
                    $term_data = get_term_by('id', $term_id, 'ad_cats');
                    ?>
                    <div class="col-md-12 col-xs-12 col-sm-12 col-lg-12">
                        <div class="sb-term-desc">
                            <div class="heading-panel"><div class="main-title text-left"><?php echo esc_html($term_data->name) . ' ' . $sb_cat_desc_title ?></div></div>
                ...
                if ($search_cat_page && $sidebar_position == 'left') {
                    ...
                    if (is_active_sidebar('adforest_cat_search')) : dynamic_sidebar('adforest_cat_search');
                    endif;
                    echo '</div></div></div>';
                }
                ?>
                                ...
                                <div class="listingTopFilterBar">
                                    <div class="col-md-7 col-xs-12 col-sm-7 no-padding">
                                        <ul class="filterAdType">
                                            ...
                                            if ($param != "") {
                                                ?><li class=""><a class="filterAdType-count" href="<?php echo adforest_returnEcho($sb_search_page); ?>"><?php echo __('Reset Search', 'adforest'); ?></a></li>
                                            <?php } ?></ul>
                                    </div>
                            ...
                            <?php if (isset($term_id) && $term_id != "") { ?>
                                ...
                                if (count($ad_cats) > 0) {
                                    ...
                                    foreach ($ad_cats as $ad_cat) {
                                        ...
                                        if ($new_params != "") {
                                            $cat_link = get_term_link($ad_cat->slug, 'ad_cats');
                                        } else {
                                            $cat_link = get_term_link($ad_cat->slug, 'ad_cats');
                                        }
                                        ...
                                        if ($rows_count > $max_rows && $show) {
                                            $show = false;
                                            $res .= '<li class="col-md-12 col-sm-12 col-xs-12 hide_cats text-center margin-top-20"><a href="javascript:void(0);" class="tax-show-more btn-theme">
                                                        ' . __('Show More ', 'adforest') . '</a></li>';
                                            $cls = 'no-display show_it';
                                        }

                                        $res .= '<li class="' . esc_attr($cls) . '">
                                                <a href="' . $cat_link . '" >
                                                ' . $ad_cat->name . ' <span>(' . $count . ')</span>
                                                </a>
                                                </li>';
                                        $rows_count++;
                                    }
                                        ...
                                        <div class="panel panel-default">
                                            <div class="panel-heading sb-panel-heading" role="tab" id="headingOnez">
                                                <div class="panel-title sb-panel-title">
                                                    ...
                                                    $title = adforest_get_taxonomy_parents($cat_id, 'ad_cats', true, '');
                                                    //$find = '&raquo;';
                                                    //$replace = '';
                                                    //$result = preg_replace("/$find/", $replace, $title, 1);
                                                    echo adforest_returnEcho($title);
                                                    /* <!--                                                    <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOnez" aria-expanded="true" aria-controls="collapseOnez" class="sb-btn">
                            ...
                            if (isset($adforest_theme['design_type']) && $adforest_theme['design_type'] == 'modern') {
                                ...
                                <div class="col-md-12 col-xs-12 col-sm-12 col-lg-12">
                                    <div class="filter-brudcrums">
                                        <span><?php echo __('Category', 'adforest') . ': ' . ucfirst(single_cat_title("", false)); ?></span>
                                    </div>
                                </div>
    <?php } ?>
                            <div class="clearfix"></div>
