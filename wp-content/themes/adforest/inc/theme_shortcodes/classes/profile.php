...
// Full Width Profile Top
        function adforest_profile_full_top() {
            ...
            if ($adforest_theme['communication_mode'] == 'both' || $adforest_theme['communication_mode'] == 'message') {
                $msgs = '<li><a href="javascript:void(0);"><div id="sb_dy_messages" class="menu-name" sb_action="my_msgs">' . __('Messages', 'adforest') . '</div></a></li>';
            }
            ...
            if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                $packages = '<li><a href="' . get_the_permalink($sb_packages_page) . '" target="_blank"><div class="menu-name" sb_action="">' . __('Packages', 'adforest') . '</div></a></li>';

                $order_history = '<li><a href="javascript:void(0);"><div class="menu-name" sb_action="my_orders">' . __('Package history', 'adforest') . '</div></a></li>';
            }
            $package_type_html = '';
            if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
			  ...
			  <div class="row">
				 <div class="col-md-5 col-sm-12 col-xs-12">
					...
					' . $rating . '
                                  ...
                                  ' . apply_filters('adforest_vendor_role_assign_button', '', $this->user_info->ID) . '
			...
			<ul>
                        <li class="ads-list-wrap">
                            ...
                            <ul class="dropdown-menu">
                              ...
                              <li><a href="javascript:void(0);"><div id="adforest-fav-ads" class="menu-name sub-list-item" sb_action="my_fav_ads">' . __('Fav Ads', 'adforest') . '</div></a></li>
                            </ul>
            ...
            if (isset($adforest_theme['sb_enable_social_links']) && $adforest_theme['sb_enable_social_links']) {
                ...
                foreach ($profiles as $key => $value) {
                    ...
                    $social_html .= '<div class="col-md-6 col-sm-6 col-xs-12">
						  ...
						  <input ' . $disabled_field . 'type="text" class="form-control margin-bottom-20" value="' . esc_attr(get_user_meta($this->user_info->ID, '_sb_profile_' . $key, true)) . '" name="_sb_profile_' . $key . '">
					   </div>';
                }
            }
...
if (!function_exists('adforest_packages')) {
    ...
    function adforest_packages() {
        ...
        $package = new Adforest\Inc\Theme_shortcodes\Classes\packages();
        ...
        if (count($biddings) > 0) {
            foreach ($biddings as $bid) {
                ...
                if ($bidhtml_style == 'style-2') {
                    ...
                    <div class="ad-bidn-img"><a href="' . adforest_set_url_param(get_author_posts_url($user_info->ID), 'type', 'ads') . '">' . '<img src="' . adforest_get_user_dp($user_info->ID, 'adforest-single-small') . '" alt="' . esc_attr__('image', 'adforest') . '" class="img-fluid"></a>
                    </div>
