...
if (!function_exists('fun2_factsshort_base_func')) {
    ...
    function fun2_factsshort_base_func($atts, $content = '') {
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                if (isset($row['numbers']) && isset($row['title'])) {
                    ...
                    $fun_html .= '<div class="col-lg-' . esc_attr($p_cols) . ' col-md-' . esc_attr($p_cols) . ' col-sm-6 col-xs-6">' . $icon_html . '<div class="number"><span class="timer" data-from="0" data-to="' . $row['numbers'] . '" data-speed="1500" data-refresh-interval="5">0</span></div><h4>' . $row['title'] . ' ' . $color_html . '</h4></div>';
                }
            }
        }
        ...
        return '<div class="funfacts custom-padding parallex" ' . $style . '>
			...
			<div class="client-3"> <img src="' . trailingslashit(get_template_directory_uri()) . 'images/as.png"  alt="' . esc_attr('image', 'adforest') . '" class="img-responsive"> </div>
         </div>';
    }
