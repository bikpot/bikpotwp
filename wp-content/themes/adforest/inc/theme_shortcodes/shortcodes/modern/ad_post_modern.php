...
if (!function_exists('ad_post_short_base_func')) {
    ...
    function ad_post_short_base_func($atts, $content = '') {
        ...
        if (isset($atts['fields']) && $atts['fields'] != '') {
            ...
            if (isset($rows) && is_array($rows) && count($rows) > 0 && count($rows[0]) > 0) {
                $total_fileds = 1;
                $extra_fields_html .= '<div class="select-package no-over-flow">
        ...
        wp_enqueue_style('adforest-dt', trailingslashit(get_template_directory_uri()) . 'css/datepicker.min.css');
        ...
        if ($rtl) {
            wp_enqueue_style('adforest-wizard-rtl', trailingslashit(get_template_directory_uri()) . 'css/rtl-wizard.css');
            if (isset($adforest_theme['post_ad_layout']) && $adforest_theme['post_ad_layout'] == 'arrows') {
                wp_enqueue_style('adforest-wizard-arrows-rtl', trailingslashit(get_template_directory_uri()) . 'css/rtl-wizard-arrows.css');
            }
        } else {
        ...
        if (isset($adforest_theme['sb_enable_comments_offer']) && $adforest_theme['sb_enable_comments_offer'] && isset($adforest_theme['sb_enable_comments_offer_user']) && $adforest_theme['sb_enable_comments_offer_user']) {
            $bidable .= '<div class="bidding-content"' . $bid_style_cat . '>';
            $bidable .= '<div class="select-package no-over-flow">
			   <div class="no-padding col-md-12 col-lg-12 col-xs-12 col-sm-12">
            ...
            if (isset($adforest_theme['bidding_timer']) && $adforest_theme['bidding_timer']) {
                $bidable .= '<div class="row biddind_div">
			       ...
			       <input class="form-control" placeholder="' . __('Click to select', 'adforest') . '" type="text" name="ad_bidding_date" id="ad_bidding_date"  value="' . $ad_bidding_date . '">
			     </div>
                            </div>';
            }
            $bidable .= '</div>';
        ...
        if (isset($is_update) && $is_update != "") {
            ...
            if (isset($adforest_theme['sb_allow_free_bump_up']) && $adforest_theme['sb_allow_free_bump_up']) {
                ...
                $bump_ad_html = '<div class="select-package no-over-flow">
                              	<div class="no-padding col-md-12 col-lg-12 col-xs-12 col-sm-12">
            ...
            if ($is_package_notification && isset($adforest_theme['sb_show_bump_up_notification']) && $adforest_theme['sb_show_bump_up_notification']) {
                $bump_ad_html = '<div class="row"><div class="col-md-12 col-sm-12 col-xs-12"><div role="alert" class="alert alert-info alert-dismissible ' . adforest_alert_type() . '">
        ...
        if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
            if (isset($adforest_theme['allow_featured_on_ad']) && $adforest_theme['allow_featured_on_ad'] && $is_feature_ad == 0 && ( get_user_meta(get_current_user_id(), '_sb_expire_ads', true) == '-1' || get_user_meta(get_current_user_id(), '_sb_expire_ads', true) >= date('Y-m-d') )) {
                if (get_user_meta(get_current_user_id(), '_sb_featured_ads', true) == '-1' || get_user_meta(get_current_user_id(), '_sb_featured_ads', true) > 0) {
                    ...
                    $simple_feature_html = '<div class="select-package no-over-flow">
                              	<div class="no-padding col-md-12 col-lg-12 col-xs-12 col-sm-12">
                                 <div class="pricing-list">
                                    <div class="row">
                                       <div class="col-md-12 col-sm-12 col-xs-12">
        ...
        if (isset($adforest_theme['map-setings-map-type']) && $adforest_theme['map-setings-map-type'] == 'google_map') {
            $loc_scropts = "<script> function adforest_location() { var options = {types: [" . $types . "], " . $stricts . "}; var input = document.getElementById('sb_user_address'); var action_on_complete = '1'; var autocomplete = new google.maps.places.Autocomplete(input, options); if( action_on_complete ){new google.maps.event.addListener(autocomplete, 'place_changed', function() { var place = autocomplete.getPlace();
        ...
        $ad_post_modern_html = '';
        $ad_post_modern_step_html = '';
        $ad_post_modern_html .= $loc_scropts . ' 
  <section class="section-padding form-wizard">
        ...
        $categories_style_html = ' <div class="row">
                                    <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
        ...
        $ad_post_modern_step_html = apply_filters('adforest_directory_ad_post_wizard_steps', $ad_post_modern_step_html);
        $ad_post_modern_html .= $ad_post_modern_step_html;
        ...
        $directory_post_args = array('is_update' => $is_update, 'ad_style' => 'modern',);
        $ad_post_modern_htmll = apply_filters('adforest_directory_ad_post_fields_frontend', '', $directory_post_args);

        $step_val = 'step-2';
        $step_val = apply_filters('adforest_directory_ad_post_wizard_step_value', $step_val);
        $ad_post_modern_html .= $ad_post_modern_htmll;
        $ad_post_modern_html .= '<div id="' . $step_val . '" class="">
					<div class="row">
					  <div class="col-md-6 col-lg-6 col-xs-12 col-sm-12">
