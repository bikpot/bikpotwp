...
if (!function_exists('shop_layout_book2_short2')) {

    function shop_layout_book2_short2() {
        vc_map(array(
            "name" => __("Shop Layout - Book 2", 'adforest'),
            "base" => "shop_layout_book2_short2_base",
            ...
            "params" => array(
                array(
                    ...
                    'description' => adforest_VCImage('shop-layout-book-2.png') . __('Ouput of the shortcode will be look like this.', 'adforest'),
                ),
                array(
...
add_action('vc_before_init', 'shop_layout_book2_short2');
if (!function_exists('shop_layout_book2_short2_base_func')) {

    function shop_layout_book2_short2_base_func($atts, $content = '') {
        ...
        if ($product_loop->have_posts()) {
            while ($product_loop->have_posts()) {
                ...
                if (get_the_post_thumbnail($product_id)) {
                    $shop_thumb = '<a href="' . esc_url(get_the_permalink($product_id)) . '">' . get_the_post_thumbnail(get_the_ID(), 'adforest-shop-book') . '</a>';
                }
                ...
                $aUrl = wc_get_page_permalink('shop');
                $p_cols = isset($p_cols) && $p_cols != '' ? $p_cols : 4;
                $html .= '<div class="col-xs-12 col-sm-6 col-lg-' . esc_attr($p_cols) . ' col-md-' . esc_attr($p_cols) . '">
                            <div class="top-bk-box">
                                <div class="top-ox-img">' . $shop_thumb . '</div>
                                <div class="top-bk-text-area">
                                    <div class="top-bk-details">
                                        ' . adforest_get_woo_stars($average_rating) . '
                                        <a class="top-bk-text-title" href="' . get_the_permalink($product_id) . '" title="' . get_the_title($product_id) . '"><h4>' . get_the_title($product_id) . '</h4></a>
                                        <p>' . $regular_price . $sale_price . ' &nbsp;</p>
                                        <a class="top-bk-text-btn"  href="' . esc_url($aUrl) . '" title="' . get_the_title($product_id) . '"><span>' . esc_html__('Shop Now', 'adforest') . '</span></a>
                                    </div>	
                                </div>
                            </div>  	
                        </div>';
        ...
        return '<section class="top-bk-ads padding-bottom-30 ' . $parallex . ' ' . $bg_color . '" ' . $style . '>
                    <div class="container">
                        <div class="row">
                             ' . $header . '
                            <div class="col-lg-12 col-xs-12 col-md-12 col-sm-12"><div class="row">' . $html . '</div></div>
...
if (function_exists('adforest_add_code')) {
    adforest_add_code('shop_layout_book2_short2_base', 'shop_layout_book2_short2_base_func');
}