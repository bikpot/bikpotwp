...
/* advertisement */
/* ------------------------------------------------ */
if (!function_exists('ad_720_short')) {
    function ad_720_short() {
        vc_map(array(
            "name" => __("Advertisement", 'adforest'),
            "description" => __("720x90 Banner", 'adforest'),
            "base" => "ad_720_short_base",
...
add_action('vc_before_init', 'ad_720_short');
if (!function_exists('ad_720_short_base_func')) {
    function ad_720_short_base_func($atts, $content = '') {
        ...
        if (isset($adforest_elementor) && $adforest_elementor) {
            ...
            $adf_code = urldecode(adforest_decode($ad_720_90));
        }
        return '<section class="advertizing '.$bg_color.'" '. $style .'>
            <div class="container">
               <div class="row">
			   		'.$header.'
                  <div class="col-md-12 col-xs-12 col-sm-12"><div class="banner">'.$adf_code.'</div></div>
               </div>
            </div>
         </section>';
    }
...
if (function_exists('adforest_add_code')) {
    adforest_add_code('ad_720_short_base', 'ad_720_short_base_func');
}