    ...
    function cats_grid_short_base_func($atts, $content = '') {
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                if (isset($row['cat']) && isset($row['icon'])) {
                    ...
                    $categories_html .= '<div class="custom-cat"><i class="' . $icon_class . '"></i><h6><a href="' . adforest_cat_link_page($row['cat'], $cat_link_page) . '">' . $category->name . '</a></h6><p>' . $sub_cat_html . '</p></div>';
        ...
        return '<section class="custom-padding categories ' . $bg_color . '" ' . $style . '>
            <div class="container">
               <div class="row">
			   		' . $header . '
			   <div class="col-md-12 col-xs-12 col-sm-12"><div class="category-grid">' . $categories_html . '</div></div>
			   </div>
            </div>
         </section>
