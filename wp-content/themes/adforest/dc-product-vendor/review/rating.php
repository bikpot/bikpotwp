...
<?php if ( $count > 0 ) {
	$star_text = __( 'Stars', 'adforest' );
	?>
    <span class="wcmp_total_rating_number"><?php echo __( sprintf( ' %s %s', $rating, $star_text ) ); ?></span>
<?php } ?>
	...
	<?php if ( $count > 0 ) { ?>
        <span itemprop="reviewRating" itemscope itemtype="http://schema.org/Rating" class="star-rating"
            ...
            <span style="width:<?php echo ( round( $rating_val_array['avg_rating'] ) / 5 ) * 100; ?>%"><strong
                        itemprop="ratingValue"><?php echo $rating; ?></strong> <?php _e( 'out of 5', 'dc-woocommerce-multi-vendor' ); ?></span>
        </span>
